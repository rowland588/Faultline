...
export type RouteName = 'home' | 'resume' | 'capture' | 'analyse' | 'present' | 'meeting' | 'log' | 'settings' | 'people'
  ...
  | 'projects' | 'projectDashboard' | 'projectSetup' | 'projectLine' | 'paceReport' | 'leverTree' | 'board' | 'pareto' | 'commissioning' | 'commissionRun';
...
export function parseRoute(hash: string): Route {
  ...
  if (segs[0] === 'project' && segs[1]) {
    ...
    if (segs[2] === 'commissioning') {
      return { name: segs[3] === 'run' ? 'commissionRun' : 'commissioning', id, query };
    }
